...
const seedRecords = {
  babag: [{ id: "REC-BAB-001", branchId: "babag", performedBy: { id: "owner-1", name: "Maria Santos" }, createdAt: "2026-08-18T16:20:00.000Z", status: "COMPLETED", adjustmentStatus: "APPLIED", adjustedAt: "2026-08-18T16:28:00.000Z", adjustedBy: { id: "owner-1", name: "Maria Santos" }, items: [
    { ingredientId: "bab-egg", name: "Egg", unit: "pc", systemQuantity: 160, physicalQuantity: 156, variance: -4, reason: "Miscount" },
    { ingredientId: "bab-bun", name: "Burger Bun", unit: "pc", systemQuantity: 12, physicalQuantity: 12, variance: 0, reason: "" },
    { ingredientId: "bab-cooking-oil", name: "Cooking Oil", unit: "ml", systemQuantity: 1800, physicalQuantity: 1750, variance: -50, reason: "Damaged" },
  ] }],
  marigondon: [{ id: "REC-MAR-001", branchId: "marigondon", performedBy: { id: "staff-1", name: "Jordan Reyes" }, createdAt: "2026-08-18T17:05:00.000Z", status: "COMPLETED", adjustmentStatus: "PENDING", items: [
    { ingredientId: "mar-egg", name: "Egg", unit: "pc", systemQuantity: 90, physicalQuantity: 92, variance: 2, reason: "Miscount" },
    { ingredientId: "mar-bun", name: "Burger Bun", unit: "pc", systemQuantity: 46, physicalQuantity: 46, variance: 0, reason: "" },
    { ingredientId: "mar-fruit-tea", name: "Passion Fruit Syrup", unit: "ml", systemQuantity: 1000, physicalQuantity: 900, variance: -100, reason: "Expired/old stock" },
  ] }],
};
...
function normaliseItems(items) {
  return items.map((item) => {
    ...
    return { ...item, systemQuantity, physicalQuantity, variance: physicalQuantity === null ? null : physicalQuantity - systemQuantity, reason: item.reason?.trim() ?? "" };
  });
}
...
export async function getInventorySnapshot(branchId) {
  ...
  return inventory.filter((item) => item.active).map(({ id, name, unit, currentQuantity }) => ({ ingredientId: id, name, unit, systemQuantity: currentQuantity, physicalQuantity: null, variance: null, reason: "" }));
}
...
export async function createReconciliation(branchId, user) {
  ...
  const record = { id: `REC-${prefix}-${String(branchRecords.length + 1).padStart(3, "0")}`, branchId, performedBy: { id: user.id, name: user.name }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), status: "DRAFT", adjustmentStatus: "NOT_APPLICABLE", items: snapshot };
  records[branchId].unshift(record);
  writeRecords(records);
  return clone(record);
}

export async function saveDraft(branchId, id, items, user) {
  assertActor(user);
...
export async function submitReconciliation(branchId, id, items, user) {
  assertActor(user);
  const target = getRecord(branchId, id);
  ...
  if (countedItems.some((item) => item.variance !== 0 && !item.reason)) throw new Error("Select a reason for every item with a variance.");
...
export async function applyAdjustment(branchId, id, user) {
  ...
  for (const item of target.record.items.filter((entry) => entry.variance !== 0)) {
    await adjustStock(branchId, item.ingredientId, { type: "SET", amount: item.physicalQuantity, reason: "Reconciliation adjustment" }, { actorRole: "OWNER" });
  }
