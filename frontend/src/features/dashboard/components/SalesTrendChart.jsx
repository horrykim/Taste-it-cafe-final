...
function SalesTrendChart({ sales, period, periodLabel }) {
  const points = sales[period] ?? sales.weekly;
  ...
  return (
    <div className="overflow-x-auto" aria-live="polite">
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-auto w-full" role="img" aria-label={`${period} sales trend`}>
          <title>{periodLabel ?? period} sales trend</title>
          ...
          {chartPoints.map((point) => <g key={point.label}><title>{point.label}: {formatCurrency(point.value)}</title><circle cx={point.x} cy={point.y} r="5" fill="white" stroke="#b85f98" strokeWidth="3" /><text x={point.x} y={chartHeight - 14} textAnchor="middle" className="fill-slate-500 text-[11px]">{point.label}</text></g>)}
      </svg>
    </div>
  );
}
