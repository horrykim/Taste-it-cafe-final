...
import { CategoryBadge, Card, IconButton, StatusBadge, Toggle } from "../../../components/ui";
import MenuImage from "./MenuImage";

function MenuItemCard({ item, categoryName, categoryColor, isOwner, onToggle, onDetails, onEdit, onDelete }) {
  return (
    <Card className="flex h-full flex-col overflow-hidden">
      <MenuImage imageUrl={item.imageUrl} alt={item.name} className="h-40 w-full" />
      <div className="flex flex-1 flex-col p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><CategoryBadge name={categoryName} colorId={categoryColor} /><h2 className="mt-3 truncate text-lg font-semibold text-slate-900">{item.name}</h2></div>
      ...
      <div className="mt-4 flex gap-2"><button type="button" onClick={() => onDetails(item)} className="flex-1 rounded-xl border border-taste-border px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#F777D1]">View details</button>{isOwner && <><button type="button" onClick={() => onEdit(item)} className="rounded-xl bg-taste-teal-soft px-3 py-2 text-sm font-semibold text-slate-800 transition hover:bg-taste-teal/30 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#F777D1]"><Pencil size={16} aria-hidden="true" /><span className="sr-only">Edit {item.name}</span></button><button type="button" onClick={() => onDelete(item)} className="rounded-xl bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700 transition hover:bg-rose-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#F777D1]" aria-label={`Delete ${item.name}`}><Trash2 size={16} /></button></>}</div>
      </div>
    </Card>
  );
}
