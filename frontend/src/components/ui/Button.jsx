...
const variants = {
  primary: "bg-taste-purple text-white shadow-sm hover:bg-taste-purple-strong",
  secondary: "bg-taste-purple-soft text-slate-800 border border-taste-purple/20 hover:bg-[#f3d6e8]",
...
export function Button({ className, variant = "primary", size = "md", loading = false, type = "button", children, disabled = false, ...props }) {
  ...
  return <button type={type} className={cn("inline-flex items-center justify-center gap-2 rounded-xl font-semibold transition-all duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-taste-purple disabled:pointer-events-none disabled:opacity-50", variants[variant], sizes[size], className)} disabled={loading || disabled} {...props}>{loading && <LoaderCircle size={16} className="animate-spin" aria-hidden="true" />}{children}</button>;
}
