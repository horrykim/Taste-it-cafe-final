        ...
        <nav aria-label="Main navigation" className="flex-1 overflow-y-auto px-4 py-6">
          ...
          <div className="space-y-1">
            {navigation.map(({ icon: Icon, label, path, children }) => {
              if (!children) return <NavLink key={path} to={path} onClick={onClose} end={path === "/app/dashboard"} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-taste-purple/30 ${isActive ? "bg-taste-purple-soft text-taste-text shadow-sm ring-1 ring-taste-purple/20" : "text-slate-600 hover:bg-slate-50 hover:text-taste-text"}`}><Icon size={19} strokeWidth={1.9} /><span>{label}</span></NavLink>;
              const isChildActive = (child) => location.pathname === child.path && (!child.search || location.search === child.search);
              const isGroupActive = children.some(isChildActive);
              const isExpanded = isGroupActive || expandedGroups[label];
              const Chevron = isExpanded ? ChevronDown : ChevronRight;
              return <div key={label}><button type="button" aria-expanded={isExpanded} aria-controls={`${label.toLowerCase().replaceAll(" ", "-")}-links`} onClick={() => setExpandedGroups((current) => ({ ...current, [label]: !isExpanded }))} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-taste-purple/30 ${isGroupActive ? "bg-taste-purple-soft text-taste-text shadow-sm ring-1 ring-taste-purple/20" : "text-slate-600 hover:bg-slate-50 hover:text-taste-text"}`}><Icon size={19} strokeWidth={1.9} /><span className="flex-1">{label}</span><Chevron size={17} aria-hidden="true" /></button>{isExpanded && <div id={`${label.toLowerCase().replaceAll(" ", "-")}-links`} className="mt-1 space-y-1 border-l border-taste-border pl-4">{children.map((child) => { const ChildIcon = child.icon; return <NavLink key={`${child.path}${child.search ?? ""}`} to={`${child.path}${child.search ?? ""}`} onClick={onClose} className={() => `flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-taste-purple/30 ${isChildActive(child) ? "bg-taste-purple-soft font-semibold text-taste-text" : "text-slate-600 hover:bg-slate-50 hover:text-taste-text"}`}>{ChildIcon && <ChildIcon size={16} strokeWidth={1.9} />}<span>{child.label}</span></NavLink>; })}</div>}</div>;
            })}
          </div>
        </nav>
